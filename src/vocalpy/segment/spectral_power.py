...
def spectrogram_ava(data: npt.NDArray, samplerate: int, nperseg: int, noverlap: int,
             min_freq: int, max_freq: int) -> tuple[npt.NDArray, npt.NDArray, npt.NDArray]:
    """Generate a spectrogram similar to how AVA does it.
    ...
    spect_min, spect_max = spect.min(), spect.max()
    spect = (spect - spect_min) / (spect_max - spect_min)
    return spect, f, t
...
def ava_like(
    ...
    """Find segments in audio, using an algorithm
    very similar to the one used by the ``ava`` package.

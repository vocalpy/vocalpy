...
class AudioFile(Base):
    ...
    spectrogram_files = relationship(
        "SpectrogramFile",
        back_populates="source_audio_file"
    )
...
annotates_table = Table(
    ...
    Column(
        "annotation_file_id",
        ForeignKey("annotation_files.id")
    ),
    Column(
        "dataset_file_id",
        ForeignKey("dataset_files.id")
...
class SpectrogramFile(Base):
    ...
    source_audio_file_id = Column(
        "audio_file_id",
        Integer,
        ForeignKey("audio_files.id")
    )
    spectrogram_parameters_id = Column(
        "spectrogram_parameters_id",
        Integer,
        ForeignKey("spectrogram_parameters.id")
    )
    dataset_file_id = Column(
        Integer,
        ForeignKey('dataset_files.id')
    ...
    source_audio_file = relationship(
        "AudioFile",
        back_populates=""
    )
    spectrogram_parameters = relationship(
        "SpectrogramParameters",
        back_populates="spectrogram_files"
    )
    dataset_file = relationship(
        "DatasetFile",
        back_populates="spectrogram_file"
    )
...
class SpectrogramParameters(Base):
    ...
    spectrogram_files = relationship(
        "SpectrogramFile",
        back_populates="spectrogram_parameters"
    )

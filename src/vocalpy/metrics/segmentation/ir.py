...
def _compute_tp(
        boundaries_hyp: npt.NDArray, boundaries_ref: npt.NDArray, tol: float = 0.01
) -> int:
    ...
    if tol < 0.:
        raise ValueError(
            f"Tolerance value ``tol`` must be a non-negative float but was: {tol}"
        )

    tp = 0
    for boundary_num, boundary_time_ref in enumerate(boundaries_ref):
        # FIXME: do we ignore multiple hits here?
        hits = (np.abs(
            scipy.spatial.distance.cdist(
                    np.array([[boundary_time_ref]]), boundaries_hyp[:, np.newaxis]
                )
            )
        < tol).sum()
        if hits > 0:
...
def compute_tp(onsets_hyp, offsets_hyp, onsets_ref, offsets_ref, tol=0.01, method='combine'):
    if method == 'combine':
        ...
        tp = 0
        all_hits = []
        for boundaries_hyp, boundaries_ref in zip(
            (onsets_hyp, offsets_hyp),
            (onsets_ref, offsets_ref)
        ):

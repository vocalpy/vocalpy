...
class TestAnnotationFile:
    ...
    def test_init_cbin_notmat_pairs(self, a_cbin_notmat_pair):
        ...
        audio_file = vocalpy.domain_model.entities.AudioFile(path=audio_path)
        annot_file = vocalpy.domain_model.entities.AnnotationFile(
            path=annot_path,
            annotates=audio_file
        )
        assert isinstance(annot_file, vocalpy.domain_model.entities.AnnotationFile)
        for attr_name, attr_type, attr_val in zip(
                ('path', 'annotates'),
                (pathlib.Path, vocalpy.domain_model.entities.AudioFile),
    ...
    def test_init_list_of_wav(self, annot_file_koumura, audio_list_wav):
        audio_files = [
            vocalpy.domain_model.entities.AudioFile(path=wav_path)
    ...
    def test_init_list_of_spect(self, annot_file_yarden, spect_list_mat):
        ...
        spect_files = [
            vocalpy.domain_model.entities.SpectrogramFile(path=mat_spect_path)
    ...
    def test_raises(self, annot_file_koumura, audio_list_wav, spect_list_mat):
        ...
        with pytest.raises(TypeError):
            vocalpy.domain_model.entities.AnnotationFile(
                path=annot_file_koumura,

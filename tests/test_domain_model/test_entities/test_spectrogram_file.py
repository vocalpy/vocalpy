...
class TestSpectrogramFile:
    def test_init(self, an_npz_spect_path):
        spect_file = vocalpy.domain_model.entities.SpectrogramFile(path=an_npz_spect_path)
        assert isinstance(spect_file, vocalpy.domain_model.entities.SpectrogramFile)
    ...
    def test_init_converter(self, an_npz_spect_path):
        ...
        spect_file = vocalpy.domain_model.entities.SpectrogramFile(path=an_npz_spect_path_str)
        assert isinstance(spect_file, vocalpy.domain_model.entities.SpectrogramFile)
        assert hasattr(spect_file, 'path')
        assert isinstance(
            getattr(spect_file, 'path'), pathlib.Path
    ...
    def test_with_spect_params(self, an_npz_spect_path, default_spect_params):
        ...
        for attr_name, attr_type, attr_val in zip(
            ...
            (pathlib.Path, vocalpy.domain_model.entities.SpectrogramParameters),

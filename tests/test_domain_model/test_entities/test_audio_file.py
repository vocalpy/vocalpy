...
class TestAudioFile:
    ...
    def test_init(self, an_audio_path):
        ...
        audio_file = vocalpy.domain_model.entities.AudioFile(path=an_audio_path)
        assert isinstance(audio_file, vocalpy.domain_model.entities.AudioFile)
    ...
    def test_init_converter(self, an_audio_path):
        ...
        audio_file = vocalpy.domain_model.entities.AudioFile(path=an_audio_path_str)
        assert isinstance(audio_file, vocalpy.domain_model.entities.AudioFile)
        assert hasattr(audio_file, 'path')
        assert isinstance(
            getattr(audio_file, 'path'), pathlib.Path
